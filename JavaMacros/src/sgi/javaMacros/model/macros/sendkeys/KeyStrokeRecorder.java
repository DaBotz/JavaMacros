...
public class KeyStrokeRecorder {
	...
	private static LuaMacrosCodeTable CODE_TABLE = LuaMacrosCodeTable.getCodeTable(true);
	...
	private boolean compressedRecording = true;
	...
	public void keyStrokeRaw(KeyEvent evt, boolean down) {
		actions.add(new MonoDirectionalAction(KeyCodeChecker.getAcceptableKeyCode(evt), down));
	}
	...
	public void keyStroke(int scanCode, boolean down) {
		synchronized (semaphore) {
			...
			boolean requiresEscape = (compressedRecording || scanCode != 32)//
			...
			if (down) {
				if (isModifierScanCode) {
					...
					case ESCAPE_SEQUENCE:
						if (lastAction.getScanCode() == scanCode)
							((EscapeSequenceAction) lastAction).pushedAgain();
						else
					...
					case TYPE_A_KEY:
						if (compressedRecording || requiresEscape)
							pushes++;
						else {
							// if (lastAction.getScanCode() == scanCode)
					...
					case MONODIRECTIONAL:
						if (lastAction.getScanCode() == scanCode) {
							if (compressedRecording || requiresEscape) {
								replaceLastAction(//
										new EscapeSequenceAction(scanCode, pushes, 0));
							} else {
				...
				if (opener != null && opener.getScanCode() == scanCode) {
					...
					case ESCAPE_SEQUENCE:
						EscapeSequenceAction esa1 = (EscapeSequenceAction) opener;
						if (hold > 3000) {
					...
					case TYPE_A_KEY:
						if (compressedRecording || requiresEscape) {
							EscapeSequenceAction esa0 = new EscapeSequenceAction(scanCode, 2, 0);
					...
					case MONODIRECTIONAL:
						...
						if (requiresEscape)
							toReplaceCurrent = new EscapeSequenceAction(scanCode);
						else
					...
					addAction(new EscapeSequenceAction(scanCode));
	...
	public void keyStroke(KeyEvent event, boolean down) {
		keyStroke(KeyCodeChecker.getAcceptableKeyCode(event), down);
	}

	public boolean isCompressedRecording() {
		return compressedRecording;
	}

	public void setCompressedRecording(boolean compressedRecording) {
		this.compressedRecording = compressedRecording;
	}
