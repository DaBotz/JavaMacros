...
JMA.chekHandled= function (deviceID, seed)
  
  
  if JMA.handled[deviceID]==nil then 
     JMA.handled[deviceID]=seed
  elseif JMA.handled[deviceID]>= 3 then
     
  else -- if JMA.handled[deviceID]>= 1 then
     JMA.handled[deviceID]=    JMA.handled[deviceID]+1
  end

  return  JMA.handled[deviceID]~= nil and JMA.handled[deviceID] > 2

end

JMacros_base_handler= function(deviceID, states , button, direction, deviceType)
       JMA.chekHandled(deviceID, 3)


      if (JMA.autonomous_num_lock[deviceID]) then
          if (button==144) then
             return
          end
       end

       if(states.directions[deviceID]== direction) then
             return
       end


       states.directions[deviceID]= direction
       states.keyStates[deviceID]= direction==1
       ...
       request=request .. 'source=' .. deviceID
...
JMA.add_device = function(deviceID, deviceID)

      lmc_device_set_name(deviceID,deviceID);
      lmc_device_set_name(deviceID,string.lower (deviceID));
      ...
      local handler= function(button, direction, deviceType)
            JMacros_base_handler(deviceID, caller, button, direction, deviceType)
      end

      lmc_set_handler( deviceID, handler)
