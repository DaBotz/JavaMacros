--[[
Some of the coe in this comes from Macrorecorder, by Pavlov Andrew

My thanks to him - this is the very first time I program something in Lua, 
and I hate it ( as any non - C-style language)

This script scan all available keyboards, assign to each a 1 byte id and then 
sends keystrokes to a Java application through an http get.

if the Java thing does not use the input, it can signal the script that it needs
tosend the keystrokes -this adds a lag, but obviates the need to remove the assigned 
handlers, waiting for that function to be avalilable
